...
La clave estructural es que $W$ está **congelado** — sus pesos no cambian durante el fine-tuning. Solo $A$ y $B$ son entrenables. El **modelo base** se preserva intacto, y la adaptación vive enteramente en las matrices pequeñas.
...
En el siguiente capítulo, exploraremos **QLoRA** — la extensión de LoRA que introduce cuantización del modelo base para reducir aún más el footprint de memoria, llevando el fine-tuning de modelos de 70B al rango de posibilidad en hardware de consumo.
